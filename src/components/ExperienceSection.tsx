...
export function ExperienceSection() {
  ...
  const displayedExperiences = isExpanded ? experiences : [];
    ...
    <section id="experiencia" className="container mx-auto px-4 py-32 relative z-10">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="relative">
          ...
          {/* Toggle Button - Purple with Black Center */}
          <div className="flex justify-center mb-16 relative z-30">
            <button
              onClick={() => setIsExpanded(!isExpanded)}
              className="group relative flex items-center gap-4 px-10 py-5 bg-black border-[3px] border-accent-purple rounded-full text-white font-bold text-lg hover:scale-105 transition-all shadow-[0_0_20px_rgba(168,85,247,0.3)] hover:shadow-[0_0_35px_rgba(168,85,247,0.5)] active:scale-95"
            >
              {isExpanded ? (
                <>Recolher Trajetória <ChevronUp size={24} className="group-hover:-translate-y-1 transition-transform" /></>
              ) : (
                <>Ver Experiências <ChevronDown size={24} className="group-hover:translate-y-1 transition-transform" /></>
              )}
